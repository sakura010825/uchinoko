...
import { getUchinoKoTechoById, updateUchinoKoTecho } from "@/lib/firebase/firestore"
...
export default function EditTechoPage() {
  const router = useRouter()
  const params = useParams()
  const techoId = params.id as string
  ...
  useEffect(() => {
    const loadTecho = async () => {
      ...
      setPersonality(Array.isArray(data.personality) ? data.personality : data.personality ? [data.personality] : [])
      setFirstPerson(data.firstPerson || "")
      setToneType(data.toneType || data.tone || "")
      setFamilyRelations(data.familyRelations && data.familyRelations.length > 0 ? data.familyRelations : [{ name: "", relation: "" }])
      ...
      <div className="container mx-auto px-4 max-w-3xl">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-gray-900">うちの子手帳を編集</h1>
          </div>
          <p className="text-gray-700 text-lg">
            愛猫の情報を更新して、より正確な翻訳を実現しましょう
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="flex gap-4">
            <Button
              type="submit"
              className="flex-1 bg-pink-500 hover:bg-pink-600 text-white font-bold py-6 text-lg shadow-md"
              disabled={loading}
            >
              {loading ? "更新中..." : "更新する"}
            </Button>
            <Link href="/techo" className="flex-1">
              <Button type="button" variant="outline" className="w-full border-gray-300 text-gray-900 hover:bg-gray-50 py-6 text-lg">
                キャンセル
              </Button>
            </Link>
          </div>
